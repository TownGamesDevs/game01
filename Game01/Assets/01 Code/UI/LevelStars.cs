...
public class LevelStars : MonoBehaviour
{
    ...
    [SerializeField] private GameObject[] _levelStarsPrefab;  // Array of level prefabs with star GameObjects as children
    [SerializeField, Range(0, 3)] private int[] _points; // Array to hold star ratings for each level (0 to 3)
    ...
    {
        for (int i = 0; i < _levelStarsPrefab.Length; i++)
        {
            // Ensure the first child (0-3) is always active
            //_levelStarsPrefab[i].transform.GetChild(0).gameObject.SetActive(true);

            // Get the star rating from the array
            int points = _points[i];

            // Disable star displays for 1 to 3 based on the star rating
            for (int j = 1; j < _levelStarsPrefab[i].transform.childCount; j++)
            {
                // Enable only the stars according to the rating
                _levelStarsPrefab[i].transform.GetChild(j).gameObject.SetActive(j <= points);
            }
        }
    }
    ...
    public void LoadStars()
    {
        for (int i = 0; i < _points.Length; i++)
        {
            ...
            _points[i] = PlayerPrefs.GetInt("StarsLevel" + i, 0);
        }
    ...
    private void UpdateTotalStars()
    {
        ...
        if (_totalStarsText != null)
        {
            for (int i = 0; i < _totalStarsText.Length; i++)
                _totalStarsText[i].text = totalStars + "/18";
        }
    }
